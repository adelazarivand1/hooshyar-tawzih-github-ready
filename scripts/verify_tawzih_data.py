...
EXPECTED_SAMPLE_COUNTS = {
    1: {"entries": 146, "issues": 143},
    2: {"entries": 100, "issues": 93},
    3: {"entries": 92, "issues": 88},
    4: {"entries": 89, "issues": 85},
}
...
def main():
    ...
    # 2. Validate the explicit product profile: a curated 427-entry sample.
    volume_counts = {}
    for v in [1, 2, 3, 4]:
        ...
        expected = EXPECTED_SAMPLE_COUNTS[v]
    ...
    assert len(data) == 427 and total_rulings == 409
    ids = [it["id"] for it in data]
    assert len(ids) == len(set(ids)), "Duplicate IDs found"
    print("  -> PASSED: Curated SAMPLE profile is complete and internally consistent.")
    ...
    print("\nALL 7 SAMPLE-PROFILE INTEGRITY TESTS PASSED SUCCESSFULLY!")
